...
def read_item(date: str):
    ...
    response = get(uri, headers={"Bmx-Token": TOKEN}, timeout=10).json()
    if "bmx" not in response:
        return {"error": "INVALID RESPONSE", "response": response}

    response = {
        f"{e.get('fecha').split('/')[-1]}-{e.get('fecha').split('/')[-2]}-{e.get('fecha').split('/')[0]}": e.get(
            "dato"
        )
        for e in response["bmx"]["series"][0]["datos"]
    }
    return response
